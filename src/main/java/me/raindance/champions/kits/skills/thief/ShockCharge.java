...
import me.raindance.champions.kits.itemskill.item.StunCharge;
...
@SkillMetadata(id = 712, skillType = SkillType.Thief, invType = InvType.DROP)
public class ShockCharge extends Drop implements ICooldown, TimeResource, IPassiveTimer {
    private StunCharge stunCharge = new StunCharge();
                ...
                if (entity.getLocation().distanceSquared(location) <= radiusSquared) {
                    if (entity instanceof Player && !isAlly(entity) && !entity.isDead()) {
                        ...
                        Bukkit.getScheduler().runTask(Main.instance, () -> {
                            Location land = trap.getLocation();
                            World world = land.getWorld();
                            world.strikeLightningEffect(land);
                            StatusApplier.getOrNew(player).applyStatus(Status.SILENCE, 4, 0);
                            StatusApplier.getOrNew(player).applyStatus(Status.SHOCK, 4, 0);
                            StatusApplier.getOrNew(player).applyStatus(Status.GROUND, 4, 0);
                        });
    ...
    public boolean drop(PlayerDropItemEvent e) {
        ...
        Item thrownTrap = stunCharge.throwItem(getPlayer(), Action.RIGHT_CLICK_AIR);

        Bukkit.getScheduler().runTaskLater(Main.instance, () -> {
            stunCharge.primeTrap(thrownTrap);
